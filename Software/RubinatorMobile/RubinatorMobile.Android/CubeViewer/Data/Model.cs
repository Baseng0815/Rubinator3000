...
namespace RubinatorMobile.Droid
{
    public class Model
    {
        ...
        public Model(Vertex[] vertices, bool useTangentSpace = false, uint[] indices = null)
        {
            ...
            GL.BufferData(BufferTarget.ArrayBuffer, new IntPtr(vertices.Count() * Marshal.SizeOf(vertices[0])), vertices, BufferUsage.StaticDraw);
            ...
            if (indices != null)
            {
                ...
                GL.BufferData(BufferTarget.ElementArrayBuffer, new IntPtr(indices.Count() * Marshal.SizeOf(indices[0])), indices, BufferUsage.StaticDraw);
